"""
P1 #6: 日本指数日线 (补全日报"亚太市场"板块)
- 日经 225 (Nikkei 225) ^N225
- 数据源: Yahoo Finance (Mac 直连可用)
- 拉每日最新, 入 intel.db
- 注: Yahoo 上 ^TOPX / ^TPX 都是错的 ticker（CBO 期权或 delisted），
      日本市场权威指数只用 N225 即可
"""
import sys
import time
...
def make_intel_items(quotes):
    ...
    for q in quotes:
        ...
        items.append({
            ...
            "published_at": now,
            "tags": ["jp", "index", q["code"].lower(), "asia", "daily"],
            "severity": 3,
            "extra": q,
        })
    return items
...
def run():
    log.info("=== JP 指数 ===")
    quotes = get_jp_quotes()
    if not quotes:
        log.warning("  ❌ 全部失败, 无数据")
        return 0, 0
    items = make_intel_items(quotes)
    saved, dups = save_intel(items, "jp_index", "commodity")
    for q in quotes:
        log.info(f"  ✅ {q['code']:8s} {q['price']:>10} {q['chg_pct']:+.2f}%")
    log.info(f"=== JP 完成: {saved} new, {dups} dup ===")
    return saved, dups
